...
class JCRPersistenceManager implements RecordPersistenceService {
    ...
    protected void activate(final Map<String, Object> properties) throws Exception {
        ...
        if (!session.nodeExists(ServiceConstants.JCR_PERSISTENCE_STORAGE_ROOT_NODE_PATH)) {
            ...
            node.addNode(ServiceConstants.JCR_PERSISTENCE_STORAGE_ROOT_NODE, ServiceConstants.JCR_NODE_TYPE_MONITORED_SERVICE_STORAGE)
    ...
    void persistRecordHolder(RecordHolder recordHolder) {
        ...
        try {
            ...
            def nodePath = ServiceConstants.JCR_PERSISTENCE_STORAGE_ROOT_NODE_PATH + '/' + recordHolder.canonicalMonitorName
            ...
            def recordHolderNode = rootStorageNode.addNode(recordHolder.canonicalMonitorName, ServiceConstants.JCR_NODE_TYPE_RECORD_HOLDER)
            ...
            recordHolder.records.each { DetailedPollResponse pollResponse ->
                ...
                def pollResponseNode = recordHolderNode.addNode(Constants.JCR_POLL_RESPONSE_NODE_STORAGE_FORMATTER.format(pollResponse.startTime), ServiceConstants.JCR_NODE_TYPE_DETAILED_POLL_RESPONSE)
